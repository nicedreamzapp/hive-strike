...
function windLitter(key,n,cols,size,dir){const p=bed(key,n,i=>({x:R(-40,W+40),y:R(-40,H+40),g:R(0,7),vs:R(.5,1.5),ph:R(0,7),c:cols[i%cols.length],s:R(.7,1.3)}));
 for(const b of p){b.x+=dir*b.vs*1.4;b.y+=Math.sin(t*.02+b.ph)*.45+.2;b.g+=.018*b.vs;
  ...
  X.save();X.translate(b.x+pxo(b.x,b.y),b.y);X.rotate(b.g);X.globalAlpha=.45;X.fillStyle=b.c;X.beginPath();X.moveTo(0,-size*b.s);X.quadraticCurveTo(size*.72*b.s,0,0,size*b.s);X.quadraticCurveTo(-size*.72*b.s,0,0,-size*b.s);X.fill();X.restore();}X.globalAlpha=1;}
...
function draw(){X.save();if(shake>0)X.translate(R(-shake,shake)*.4,R(-shake,shake)*.4);
 ...
 for(const b of bullets){const bd=dep(b.y)*1.1;
  ...
  if(b.k==='honey'){X.save();X.translate(b.x+pxo(b.x,b.y),b.y);X.rotate(Math.atan2(b.vy,b.vx)+Math.PI/2);if(b.evo)X.scale(1.45,1.45);X.fillStyle='rgba(255,179,0,.22)';ell(0,b.r*1.8,b.r*.9,b.r*2.6);X.fill();X.fillStyle='rgba(255,220,120,.35)';ell(0,b.r*2.6,b.r*.35,b.r*1.2);X.fill();X.fillStyle=rg(0,0,b.r,'#fff8d0','#ff9500');X.beginPath();X.moveTo(0,-b.r*2);X.quadraticCurveTo(b.r*1.15,b.r*.2,0,b.r*1.2);X.quadraticCurveTo(-b.r*1.15,b.r*.2,0,-b.r*2);X.fill();X.fillStyle='rgba(255,255,255,.85)';ell(-b.r*.35,-b.r*.5,b.r*.25,b.r*.55,.2);X.fill();X.fillStyle='rgba(180,90,0,.35)';ell(b.r*.3,b.r*.5,b.r*.3,b.r*.35);X.fill();X.restore();}
  else if(b.k==='stinger'){const pw=(1+Math.sin(t*.9+b.y*.05)*.25)*(b.evo?1.25:1);X.strokeStyle='rgba(240,220,255,.9)';X.lineWidth=1.3;X.beginPath();for(const sg of[-1,1]){let yy=b.y-22;X.moveTo(b.x+sg*b.r*pw*.4,yy);while(yy<b.y+22){yy+=R(5,9);X.lineTo(b.x+sg*b.r*pw*R(.6,1.8),yy);}}X.stroke();X.shadowColor='#c77dff';X.shadowBlur=16;const g=X.createLinearGradient(b.x-b.r*pw,0,b.x+b.r*pw,0);g.addColorStop(0,'rgba(199,125,255,0)');g.addColorStop(.4,'rgba(230,190,255,.9)');g.addColorStop(.5,'#fff');g.addColorStop(.6,'rgba(230,190,255,.9)');g.addColorStop(1,'rgba(199,125,255,0)');X.fillStyle=g;X.fillRect(b.x-b.r*pw,b.y-22,b.r*2*pw,44);X.shadowBlur=0;}
  else if(b.k==='water'){X.save();X.translate(b.x+pxo(b.x,b.y),b.y);X.fillStyle=rg(0,0,b.r*1.4,'#fff0c0','#ff8a00');X.beginPath();X.moveTo(0,-b.r*2.2);X.quadraticCurveTo(b.r*1.3,0,0,b.r*1.2);X.quadraticCurveTo(-b.r*1.3,0,0,-b.r*2.2);X.fill();X.fillStyle='rgba(255,255,255,.8)';ell(-b.r*.35,-b.r*.4,b.r*.3,b.r*.6);X.fill();X.restore();}
  else if(b.k==='wax'){X.save();X.translate(b.x+pxo(b.x,b.y),b.y);const wob=Math.sin(t*.4+b.y*.1)*.15;X.fillStyle=rg(0,0,b.r,'#fff4c8','#e0a020');ell(0,0,b.r*(1+wob),b.r*(1-wob));X.fill();ell(0,b.r*.9,b.r*.35,b.r*.5);X.fill();X.fillStyle='rgba(255,255,255,.6)';ell(-b.r*.3,-b.r*.35,b.r*.3,b.r*.22);X.fill();X.restore();}
  else if(b.k==='thorn'){X.save();X.translate(b.x+pxo(b.x,b.y),b.y);X.rotate(Math.atan2(b.vy,b.vx));X.fillStyle=rg(0,0,7,'#ffe0a0','#6a4010');X.beginPath();X.moveTo(9,0);X.lineTo(-6,-3);X.lineTo(-4,0);X.lineTo(-6,3);X.closePath();X.fill();X.restore();}
  else if(b.k==='rain'){X.save();X.translate(b.x+pxo(b.x,b.y),b.y);X.fillStyle='rgba(255,204,102,.3)';ell(0,-b.r*2,b.r*.6,b.r*3);X.fill();X.fillStyle=rg(0,0,b.r,'#fff8d0','#ff9500');X.beginPath();X.moveTo(0,-b.r*2.2);X.quadraticCurveTo(b.r*1.2,0,0,b.r*1.2);X.quadraticCurveTo(-b.r*1.2,0,0,-b.r*2.2);X.fill();X.restore();}
  else if(b.k==='lure'){X.save();X.translate(b.x+pxo(b.x,b.y),b.y);const pl=1+Math.sin(t*.3)*.2,rr=(t*3)%170;X.strokeStyle=`rgba(255,122,217,${1-rr/170})`;X.lineWidth=2;ell(0,0,rr,rr*.8);X.stroke();X.fillStyle=rg(0,0,b.r*2*pl,'rgba(255,200,240,.7)','rgba(255,122,217,0)');ell(0,0,b.r*2*pl,b.r*2*pl);X.fill();X.fillStyle='#fff';ell(0,0,b.r*.5,b.r*.5);X.fill();X.restore();}
  else if(b.k==='grenade'||b.k==='shard'){X.save();X.translate(b.x+pxo(b.x,b.y),b.y);X.rotate(t*.2);X.fillStyle=rg(0,0,b.r,'#ffe0a0','#c06010');X.beginPath();for(let k=0;k<6;k++){const a=k*Math.PI/3;X.lineTo(Math.cos(a)*b.r,Math.sin(a)*b.r);}X.closePath();X.fill();if(b.k==='grenade'){X.strokeStyle='rgba(255,255,255,.6)';X.lineWidth=1;X.stroke();}X.restore();}
  else if(b.k==='wall'){X.save();X.translate(b.x+pxo(b.x,b.y),b.y);const g=X.createLinearGradient(-b.half,0,b.half,0);g.addColorStop(0,'rgba(200,160,80,0)');g.addColorStop(.15,'rgba(230,190,100,.9)');g.addColorStop(.85,'rgba(230,190,100,.9)');g.addColorStop(1,'rgba(200,160,80,0)');X.fillStyle=g;X.fillRect(-b.half,-7,b.half*2,14);X.fillStyle='rgba(120,70,10,.8)';for(let k=-b.half+8;k<b.half-4;k+=14){X.beginPath();for(let j=0;j<6;j++){const a=j*Math.PI/3;X.lineTo(k+Math.cos(a)*5,Math.sin(a)*5);}X.closePath();X.fill();}X.globalAlpha=Math.min(1,b.life/40);X.strokeStyle='#fff8d0';X.lineWidth=1.2;X.strokeRect(-b.half,-7,b.half*2,14);X.restore();}
  else if(b.k==='lance'){X.save();X.translate(b.x+pxo(b.x,b.y),b.y);X.fillStyle='rgba(255,224,102,.25)';ell(0,14,9,40);X.fill();const g=X.createLinearGradient(0,-34,0,26);g.addColorStop(0,'#fff');g.addColorStop(.3,'#ffe066');g.addColorStop(1,'rgba(255,160,0,0)');X.fillStyle=g;X.beginPath();X.moveTo(0,-36);X.lineTo(6,-8);X.lineTo(4,26);X.lineTo(-4,26);X.lineTo(-6,-8);X.closePath();X.fill();X.fillStyle='rgba(255,255,255,.9)';ell(0,-20,1.6,10);X.fill();X.strokeStyle='rgba(255,240,160,.8)';X.lineWidth=1;X.beginPath();for(let k=0;k<3;k++){const a=t*.2+k*2.1;X.moveTo(Math.cos(a)*8,Math.sin(a)*8-8);X.lineTo(Math.cos(a)*13,Math.sin(a)*13-8);}X.stroke();X.restore();}
  else if(b.drone){X.save();X.translate(b.x+pxo(b.x,b.y),b.y);X.rotate(Math.atan2(b.vy,b.vx)+Math.PI/2);X.globalCompositeOperation='source-over';X.fillStyle='rgba(0,0,0,.22)';ell(4,8,7,3);X.fill();bee(0,0,6,'#ffd23f','#1a1a1a','#fff');X.restore();X.globalCompositeOperation='lighter';}
  else if(b.k==='static'){X.save();X.translate(b.x+pxo(b.x,b.y),b.y);const pl=1+Math.sin(t*.4)*.15;X.fillStyle=rg(0,0,b.r*2.2*pl,'rgba(230,200,255,.55)','rgba(150,80,255,0)');ell(0,0,b.r*2.2*pl,b.r*1.8*pl);X.fill();X.fillStyle='rgba(200,150,255,.8)';for(let k=0;k<6;k++){const a=k*1.047+t*.1;ell(Math.cos(a)*b.r*.7,Math.sin(a)*b.r*.5,b.r*.45,b.r*.35);X.fill();}X.fillStyle='#fff';ell(0,0,b.r*.35,b.r*.35);X.fill();X.restore();
 ...
 for(const b of ebullets){b.t=(b.t||0)+1;const r=b.r,dd=dep(b.y);X.save();X.translate(b.x+pxo(b.x,b.y),b.y);X.scale(dd,dd);X.fillStyle='rgba(0,0,0,.25)';ell(4,7,r*1.4,r*.8);X.fill();
  ...
  // one danger language for every kind: magenta (nothing in nature is), black outline for
  // bright ground, white-hot core for dark ground, drawn ~40% bigger than the hitbox.
  else{
  X.globalCompositeOperation='lighter';
  X.fillStyle=rg(0,0,r*1.8,'rgba(255,60,220,.9)','rgba(255,40,200,0)');ell(0,0,r*1.8,r*1.8);X.fill();
  X.globalCompositeOperation='source-over';
  X.strokeStyle='rgba(0,0,0,.85)';X.lineWidth=2.8;ell(0,0,r*1.45,r*1.45);X.stroke();
  X.strokeStyle='#ff3bd4';X.lineWidth=1.8;ell(0,0,r*1.45,r*1.45);X.stroke();
  X.fillStyle='rgba(255,255,255,.95)';ell(0,0,r*.42,r*.42);X.fill();}
 ...
 if(state==='play'&&!P.dead&&(P.inv===0||(t>>2)&1)){
  ...
  {const L=P.lvl,wc=(WEAPONS[P.wpn]||{}).col||'#ffd23f',d=P.dep||1,pu=.5+.5*Math.sin(t*.15);
   ...
   const hs=SPR.hero;if(hs&&hs.width){const tilt=clamp((P.x-P.px)*.035,-.35,.35);emboss(P.x,P.y,120*d,()=>drawHero(hs,60*d,tilt),{alt:1.2});}
