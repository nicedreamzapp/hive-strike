...
function rg(x,y,r,c1,c2){if(LOW3)return /,\s*0\)\s*$/.test(c2)?'rgba(0,0,0,0)':c1;   // bare bones: flat bodies, no glows
 const g=X.createRadialGradient(x-r*.3,y-r*.3,r*.1,x,y,r);g.addColorStop(0,c1);g.addColorStop(1,c2);return g;}
...
function drawBoss(b,portrait){const real=LV().boss,i=BARCH[real];
 ...
 else if(SPR['boss'+real]){const bim=SPR['boss'+real],bh=BSIZE[real],bw=bh*(bim.naturalWidth||1)/(bim.naturalHeight||1);
  // the scratch box used to be a fixed 300: the 768x600 dobsonfly, the 768x627 boss and the atlas moth
  // drew wider than that and came back with their wingtips and legs sliced off (Matt, 9/2)
  const bbox=Math.ceil(Math.max(bw,bh)*1.16)+8;
  emboss(b.x,b.y,bbox,()=>{const sc=(1+(b.rage?Math.sin(b.t*.3)*.03:0))*(1+Math.sin(b.t*.12)*.025);X.scale(sc,sc);X.rotate(Math.sin(b.t*.07)*.04);drawSprite(SPR['boss'+real],BSIZE[real],faceRot('boss'+real));},{filter:b.fl>0?'brightness(2) saturate(.5)':(b.camo?'opacity(0.13) blur(1px)':(b.rage?'saturate(1.5) contrast(1.15)':null)),alt:portrait||b.camo?0:1});}
