# Hive Strike — store listing copy

Paste-ready text for App Store Connect and the Google Play Console.
Screenshots: `store/screens/` (1290x2796, Apple 6.7") and `store/screens_play/` (1080x1920, Play phone).

---

## Name

**Hive Strike**

(30 characters max on Apple, 30 on Play. This is 11.)

## Subtitle — Apple, 30 characters

**One bee. Sixteen worlds.**

## Short description — Play, 80 characters

**One bee against sixteen worlds of bugs. Free for a month, then 99c forever.**

---

## Promotional text — Apple, 170 characters, editable without review

New: photoreal bugs re-cut by hand, wings that beat, and a kill that finally pops.
Free for thirty days, then one payment of 99c. No subscription, no adverts.

---

## Full description

You are the last bee out of a burning hive.

Sixteen worlds stand between you and the queen who took it: a sunlit meadow, a moonlit
pine wood, the inside of the comb itself, a volcano floor, a rooftop after dusk, a
crystal cavern lit only by its own stones. Each one is a painting, and each one has a
bug in it that a camera would recognise.

Fly with one finger. The bee follows your thumb, so both hands stay out of the way of
the screen. Sixteen bosses change their job halfway through the fight rather than just
getting angrier: the centipede mother splits in two and pinches, the scorpion plants
its tail as a turret and flanks you with its body, the atlas moth turns its wings into
shields you have to break.

Sixteen guns, each with five levels and a look of its own. A honey fan for a crowd, a
stinger beam for one thing you want gone, seeker pollen that never misses, a resin wall
you hide behind, a swarm of drones that hunt for you.

Forty-eight bugs, and a Bug-Dex that fills in as you beat them, each card carrying one
true fact about the real animal.

**One payment.** Play the whole game free for thirty days. After that, 99c unlocks it
forever, on every device signed in to your account. There is no subscription, nothing
else to buy, and no adverts anywhere in it.

- Sixteen hand-painted worlds and sixteen bosses
- Sixteen weapons, five levels each
- A Bug-Dex of forty-eight real insects
- Daily runs and eight contracts that change the rules
- One thumb, portrait, no tutorial to sit through
- Plays offline

---

## Keywords — Apple, 100 characters, comma separated, no spaces

shmup,bullet,hell,arcade,shooter,bee,bug,insect,retro,boss,wave,dodge,space,one,hand

## Category

Primary: Games / Arcade
Secondary (Apple): Games / Action
Play tags: Arcade, Action, Single player, Offline

## Age rating

- Apple: 9+ (infrequent cartoon or fantasy violence)
- Play / IARC: Everyone 10+ — fantasy violence, no blood, no gore, no purchases besides
  the single unlock, no ads, no user content, no data collection

---

## In-app purchase — both stores

| field | value |
|---|---|
| Product id | `com.nicedreamz.hivestrike.unlock` |
| Type | Apple: Non-Consumable. Play: In-app product, one-time |
| Reference name | Hive Strike Full Unlock |
| Display name | Unlock Hive Strike |
| Description | Unlocks the whole game forever. One payment, no subscription. |
| Price tier | 0.99 USD |

Review note for Apple, paste into App Review Information:

> The app is free to play in full for thirty days. After that a single non-consumable
> in-app purchase unlocks it permanently. There is no subscription. The paywall has a
> Restore Purchase button on it. To reach the paywall without waiting, delete the app's
> data or set the device date forward thirty days.

---

## What's new — first release

First release. Sixteen worlds, sixteen bosses, forty-eight bugs, one bee.
Free for thirty days, then one payment of 99c, forever.

---

## Support and marketing URLs

- Support: (needs a page — a mailto: link is acceptable to both stores)
- Marketing: (optional)
- Privacy policy: **required by both stores** — `store/privacy.html` is written and
  needs to be published at a public URL before submitting.
